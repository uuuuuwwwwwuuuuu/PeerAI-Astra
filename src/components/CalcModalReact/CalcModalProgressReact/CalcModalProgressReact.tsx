...
import styles from './CalcModalProgressReact.module.scss';
...
const CalcModalProgressReact: FC<Props> = ({ title, unitType, setProgressValue }) => {
    ...
    const handleChange = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        const exponentialValue =
            Math.exp((linearValue / progressParams.maxValue) * progressParams.exponentC) - 1;
        const scaledValue = Math.round(
            (exponentialValue / (Math.exp(progressParams.exponentC) - 1)) * progressParams.maxValue
        );
    ...
    const getLinearValue = useCallback(
        (expValue: number) => {
    ...
    return (
        <div className={styles.progress_wrapper}>
            <div className={styles.progress_info_line}>
